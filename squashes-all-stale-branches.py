...
def write_to_file(content):
    with open("squashesbrancheslist.csv",'a+') as file:
        file.write(content+"\n")
...
def deletebranch(repo_name,final_branch):
    ...
    for finalurl,days in delete_urllist.items():
        if (days < -21):
            response= requests.delete(finalurl, auth=HTTPBasicAuth('jenkins_scm', 'aa0d3eb79fcc2b08a4d16a3438d6e3715cccafd1'),verify=False)
            if (response.status_code == 204):
                print ("Branch succesfully deleted %s" %(finalurl))
            else:
                print ("url is exited with %s" %(response.status_code))
